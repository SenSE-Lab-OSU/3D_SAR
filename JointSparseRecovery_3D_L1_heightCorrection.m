...
function JointSparseRecovery_3D_L1_heightCorrection(im_final,xImage,yImage,f1,azimuthVals,elev1,idxImage)
...
wavelengthCenter = max(cspeed./meanF);
meanElev = sort(meanElev);
minDiffElev= min(diff(meanElev));
...
D_z = wavelengthCenter/(2*deg2rad(minDiffElev));  % Assumes that the elevation angles are specified in degrees. 
                                                  %If angle is already in
                                                  %radians then please use 
                                                  % D_z = wavelengthCenter/(2*minDiffElev);

% height of scene used in GOTCHA [-40m,40m], change this according to scene size
...
sc_points_layOver(3,:) = arrayfun(@(i)zGrid(aa(i),bb(i),cc(i)),(1:length(aa)).'); %+ shiftZ 
