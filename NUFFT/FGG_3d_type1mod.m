function  F = FGG_3d_type1mod(f,knots,N,accuracy,GridListx, GridListy, GridListz)
%Description:
...
FFF = F_tau(round(.5*(R-1)*Nx)+1:end,round(.5*(R-1)*Ny)+1:end,round(.5*(R-1)*Nz)+1:end );
clear F_tau;
FFF1 = FFF(1:Nx,1:Ny,1:Nz);
clear FFF;
...
F=FFF1.*D/(M*R*R*R); %Note that the scaling factor is 1/M, which is 
